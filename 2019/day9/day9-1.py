...
opcode_block_sizes = [
    ...
    4, # Opcode 8: equals
    2, # Opcode 9: change relative base
]
...
class Program:
    ...
    def __init__(self, code, manual_inputs, print_output, memory_size):
        self.code = code + [0] * memory_size
    ...
    def get_parameters(self):
        ...
        size = opcode_block_sizes[self.opcode]
    ...
    def step(self):
        ...
        self.opcode, c, b, a = solve_opcode(opcode_string)

        if self.opcode == 99: return
        elif self.opcode == 0:
            self.pointer += 1
            return
        ...
        self.process_step(parameters, [c, b, a])

    def run_till_output(self):
        while self.opcode != 99:
            try:
                output = self.outputs[self.output_pointer]
    ...
    def memory_read(self, index, parameters, modes):
        try:
            mode = modes[index]
            ...
            if mode == 0:  # Position mode
                ...
                return self.code[parameter + self.relative_base]
        except:
            return None

    def memory_write(self, address, value, mode):
        if mode == 0:
            self.code[address] = value
        elif mode == 2:
            self.code[address + self.relative_base] = value

    def process_step(self, parameters, modes):
        pointer_changed = False

        value1 = self.memory_read(index=0, parameters=parameters, modes=modes)
        value2 = self.memory_read(index=1, parameters=parameters, modes=modes)
        value3 = self.memory_read(index=2, parameters=parameters, modes=modes)

        if self.opcode == 1:
            self.memory_write(address=parameters[2], value=value1 + value2, mode=modes[2])
            ...
            if self.manual_inputs:
                try:
                    ...
                    self.memory_write(address=parameters[0], value=value, mode=modes[0])
                except:
                    self.process_step(parameters, modes)
            else:
                try:
                    ...
                    print('No input found, quiting program...')
                    self.opcode = 99
            ...
            self.memory_write(address=parameters[2], value=value, mode=modes[2])
        ...
        if not pointer_changed:
            self.pointer += opcode_block_sizes[self.opcode]


program = Program(code.copy(), False, True, 1000)

program.input(2)
program.run()


# Well this went pretty bad t.b.h, I could have done a better job with this but didn't really spend much time on it.
# It also was pretty hard to understand at some point.
# Day 9-1 and 9-2 are in 1 day because I only needed to change 1 number
