...
def execute(data, inputs):
    ...
    def process(opcode, parameters, modes):
        global current_total_index, current_input_index, outputs
...
sequences = [''.join(p) for p in itertools.permutations('01234')]
def test_sequence(sequence):
    ...
    for phase in sequence:
        prev_output = execute(program.copy(), [phase, prev_output])[0]
        outputs = []
