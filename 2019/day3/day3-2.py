...
wire1_path = wire1.split(',')
wire2_path = wire2.split(',')

buffer = [[0 for x in range(12000)] for y in range(12000)]
cross_points = []


def is_occupied(x, y, wire):
    global buffer
    return buffer[y][x] and int(str(buffer[y][x])[0]) != wire


def calc_wire(path, wire):
    current_x = 0
    current_y = 0
    steps_taken = 0
    for dir_length in path:
        direction = dir_length[0]
        length = int(dir_length[1:])

        for i in range(0, length):
            current_x += 1 if direction == 'R' else 0
            current_y += 1 if direction == 'U' else 0
            current_x -= 1 if direction == 'L' else 0
            current_y -= 1 if direction == 'D' else 0
            steps_taken += 1

            if is_occupied(current_x, current_y, wire):
                cross_points.append((current_x, current_y, int(buffer[current_y][current_x][1:]) + steps_taken, str(buffer[current_y][current_x][1:]) + '+' + str(steps_taken)))

            buffer[current_y][current_x] = str(wire) + str(steps_taken)


calc_wire(wire1_path, 1)
calc_wire(wire2_path, 2)
...
# Worked out pretty bad, made it in about 1,5 hours. It also isn't efficient because the 2d array is made statically,
# it should be made dynamically
